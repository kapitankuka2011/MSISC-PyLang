...
class IllegalCharError(Error):
  def __init__(self, pos_start, pos_end, details):
    super().__init__(pos_start, pos_end, 'InvalidCharacterError', details)
...
class NotDefinedError(Error):
  def __init__(self, pos_start, pos_end, details):
    super().__init__(pos_start, pos_end, 'NotDefinedError', details)

class IllegalOperation(Error):
  def __init__(self, pos_start, pos_end, details):
    super().__init__(pos_start, pos_end, 'IllegalOperation', details)
  ...
  def check_args(self, arg_names, args):
    ...
    if len(args) > len(arg_names):
      return res.failure(RTError(
        ...
        f"{len(args) - len(arg_names)} too many arguments in {self}",
        self.context
      ))
    ...
    if len(args) < len(arg_names):
      return res.failure(RTError(
        self.pos_start, self.pos_end,
        f"{len(arg_names) - len(args)} not enough arguments for {self}",
        self.context
      ))
...
class BuiltInFunction(BaseFunction):
  ...
  def execute_input_msg(self, exec_ctx):
    text = input(str(exec_ctx.symbol_table.get('value')))
    return RTResult().success(String(text))
  execute_input_msg.arg_names = ['value']

  def execute_wait(self, exec_ctx):
    #os.system("PING localhost -n " + str(exec_ctx.symbol_table.get('value')) + " >nul")
    os.system("PING localhost -n " + str(exec_ctx.symbol_table.get('value')) + " >nul" if os.name == "nt" else "sleep " + str(exec_ctx.symbol_table.get('value')))
    return RTResult().success(Number.null)
  ...
  def execute_py(self, exec_ctx):
    try:
      eval(str(exec_ctx.symbol_table.get('value')))
    except SyntaxError as e:
      print(f"Python SyntaxError: {e}")
    except Exception as e1:
      print(f"Python Exception: {e1}")
    return RTResult().success(Number.null)
  execute_py.arg_names = ['value']
...
BuiltInFunction.input_msg   = BuiltInFunction("input_msg")
BuiltInFunction.pause       = BuiltInFunction("pause")
BuiltInFunction.cmd         = BuiltInFunction("cmd")
BuiltInFunction.py         = BuiltInFunction("py")
...
global_symbol_table.set("print_red", BuiltInFunction.print_ret)
...
global_symbol_table.set("input_msg", BuiltInFunction.input_msg)
global_symbol_table.set("pause", BuiltInFunction.pause)
global_symbol_table.set("cmd", BuiltInFunction.cmd)
global_symbol_table.set("system", BuiltInFunction.cmd)
global_symbol_table.set("terminal", BuiltInFunction.cmd)
global_symbol_table.set("execute", BuiltInFunction.cmd)
